...
class Connection:
    """wrapper for TCP/USB connections"""
    
    def __init__(self, handler, local_id, remote_id, timeout_ms):
    ...
    def _Send(self, command, arg0, arg1, data=''):
        message = Message(command, arg0, arg1, data)
        message.Send(self.handler, self.timeout_ms)
  ...
  def ReadUntil(self, *expected_cmds):
    ...
    cmd, remote_id, local_id, data = AdbMessage.Read(
        self.handler, expected_cmds, self.timeout_ms)
...
class Message:
    ...
    def Send(self, handler, timeout_ms=None):
        """Send this message over USB."""
        handler.BulkWrite(self.Pack(), timeout_ms)
        handler.BulkWrite(self.data, timeout_ms)

    @classmethod
    def Read(cls, handler, expected_cmds, timeout_ms=None, total_timeout_ms=None):
        """Receive a response from the device."""
        total_timeout_ms = handler.Timeout(total_timeout_ms)
        start = time.time()
        while True:
            msg = handler.BulkRead(64, timeout_ms)
        ...
        if data_length > 0:
            data = handler.BulkRead(data_length, timeout_ms)
