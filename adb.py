...
class ADBProcessError(subprocess.CalledProcessError):
...
class ConnectionError(ADBProcessError, EnvironmentError):
    """Exception for connection errors."""
    pass


class DeviceError(ADBProcessError, EnvironmentError):
    """Exception for devices errors."""
    pass
...
def adb_exec(*args, **kwargs):
    ...
    try:
        ...
        raise ADBProcessError(errmsg)

    if proc.wait():
        raise DeviceError(proc.returncode, ' '.join(args), stderr.strip('\n'))
...
def connect(host, port=5555):
    ...
    for line in adb_exec('connect', ':'.join([host, str(port)])):
...
def sync(directory=None, list_only=False):
    ...
    should be synced.  If *list_only* is True then sync() will return a list of
    of simulated file pushes.

    list_only means list but don't copy).
    """
    args = ['sync']
